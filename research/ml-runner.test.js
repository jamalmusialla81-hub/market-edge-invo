...
const alteredFuture=rows.map(item=>({...item,targets:{...item.targets}}));
alteredFuture[26].targets={tp1BeforeSl:1,finalR:99};
const futureCheck=evaluate(alteredFuture);
assert.equal(first.classificationPredictions[0],futureCheck.classificationPredictions[0]);
assert.equal(first.regressionPredictions[0],futureCheck.regressionPredictions[0]);
