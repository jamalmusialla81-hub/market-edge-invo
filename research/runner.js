...
async function replay(asset,options={}){
  const progress=options.progress||await status(),prior=progress.states.find(row=>row.asset===asset),cursor=Number(options.cursor||prior?.cursor_timestamp||Date.now()-((WARMUP+CHUNK+OUTCOME_BARS)*BASE_MS)),sourceHash=prior?.source_dataset_hash||hash([asset,'COINBASE',cursor]);
  const start=cursor-WARMUP*BASE_MS,end=cursor+(CHUNK+OUTCOME_BARS)*BASE_MS,candles=await coinbase(asset,start,end);if(candles.length<WARMUP+CHUNK)throw new Error(`${asset} has ${candles.length} valid candles; requires ${WARMUP+CHUNK}`);
