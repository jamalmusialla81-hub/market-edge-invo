...
let coinbaseHeaders;const coinbase=await fetchAssetCandles({asset:'BTC',symbol:'BTCUSDT',exchange:'COINBASE',coinbaseProduct:'BTC-USD'},async(_url,options)=>{coinbaseHeaders=options.headers;return new Response(JSON.stringify(coinbaseRows),{status:200});},now);
assert.equal(coinbase.exchange,'COINBASE');assert.equal(coinbase.symbol,'BTC-USD');
assert.match(coinbaseHeaders['user-agent'],/MarketEdgeResearch/);
