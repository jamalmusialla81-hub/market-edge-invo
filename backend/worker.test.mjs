...
const terminalProxyTarget={...dataGapTarget,outcome_source:'BINANCE_PROXY'};
response=await handleRequest(request('/v1/research/ingest',{operation:'historical_rank_outcome_recovery_commit',scan_id:'rank-v2-fixture',outcomes:[{candidate_id:'rank-v2-candidate-fixture',targets:terminalProxyTarget,outcome_hash:stableHash({candidate_id:'rank-v2-candidate-fixture',targets:terminalProxyTarget})}]},{authorization:'Bearer test-research-secret'}),researchEnv,{},{ });
assert.equal(response.status,200);body=await response.json();assert.equal(body.terminal_unresolved,1);assert.equal(body.immutable_inputs,true);
