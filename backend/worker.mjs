...
function historicalRankCandidate(input,scan){
  ...
  if(!id||!['BTC','ETH','SOL','XRP','DOGE','LTC'].includes(asset)||!['long','short'].includes(direction)||!strategy||candidateCount!==scan.candidateCount||!featureHash||!candidateHash||ml!=='ML_NOT_AVAILABLE_FOR_HISTORICAL_TIMESTAMP'||(rank!==null&&(!Number.isInteger(rank)||rank<1||rank>candidateCount)))throw Object.assign(new Error('Invalid historical rank candidate'),{status:400,code:'HISTORICAL_RANK_CANDIDATE_INVALID'});
