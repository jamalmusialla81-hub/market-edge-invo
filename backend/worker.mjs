...
async function mlEvolution(db,env={}){const [active,challengers,reports,lifecycle]=await Promise.all([activeMlModel(db),shadowChallengers(db,env),db.prepare(`SELECT report_id,model_id,report_json,created_at FROM ml_promotion_readiness_reports ORDER BY created_at DESC LIMIT 30`).all().catch(()=>({results:[]})),lifecycleStatuses(db)]),latestReports=new Map();for(const row of reports?.results||[])if(!latestReports.has(row.model_id))latestReports.set(row.model_id,{reportId:row.report_id,...parseModelMetadata(row.report_json),createdAt:row.created_at});return{active:{id:active.id||null,status:active.status||'UNAVAILABLE',generation:null,weight:'unchanged'},challengers:challengers.map(model=>({id:model.id,status:lifecycle.get(model.id)?.to_status||model.status,generation:model.generation,datasetHash:model.datasetHash,report:latestReports.get(model.id)||null})),autoChampionPromotion:false};}
