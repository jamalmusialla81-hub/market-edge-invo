...
import {accountError,accountState,acceptTrade,closeTrade,entitlement,finalizeScan,listTrades,releaseScan,resolvePrincipal,reserveScan,storeRecommendation,successfulScan} from './account-core.mjs';
...
function corsHeaders(request,env) {
  ...
  return allowedOrigins(env).includes(origin)?{'access-control-allow-origin':origin,'vary':'Origin','access-control-allow-methods':'GET,POST,OPTIONS','access-control-allow-headers':'content-type,authorization,x-market-edge-token','access-control-allow-credentials':'true','access-control-max-age':'86400'}:{};
}
...
function historicalRankHash(value){return stableHash(value);}
function historicalRankSnapshot(payload){
  const snapshot=payload?.snapshot&&typeof payload.snapshot==='object'?payload.snapshot:{},scanId=safeText(snapshot.scan_id,180),timestamp=Number(snapshot.scan_timestamp),dataTimestamp=Number(snapshot.data_timestamp),sourceHash=safeText(snapshot.source_dataset_hash,120),universe=Array.isArray(snapshot.eligible_universe)?snapshot.eligible_universe.map(safeAsset).filter(asset=>asset!=='UNKNOWN'):[],candidateCount=Number(snapshot.candidate_count),cadence=Number(snapshot.scan_cadence_ms),mode=safeText(snapshot.universe_mode,80);
  if(!scanId||![timestamp,dataTimestamp,candidateCount,cadence].every(Number.isFinite)||timestamp<=0||dataTimestamp!==timestamp||candidateCount<0||candidateCount>96||cadence<300000||!sourceHash||!['HISTORICAL_DATA_UNIVERSE_PROXY','HISTORICAL_INVO_UNIVERSE'].includes(mode)||!universe.length)throw Object.assign(new Error('Invalid historical rank snapshot'),{status:400,code:'HISTORICAL_RANK_SNAPSHOT_INVALID'});
  const row={scanId,timestamp,dataTimestamp,mode,universe,sourceHash,cadence,engine:safeText(snapshot.engine_version,120),strategy:safeText(snapshot.strategy_version,120),quant:safeText(snapshot.quant_version,120),ml:safeText(snapshot.ml_version,120),feature:safeText(snapshot.feature_version,120),candidateCount,snapshotHash:safeText(snapshot.snapshot_hash,120)};
  const expected=historicalRankHash({scan_id:scanId,scan_timestamp:timestamp,data_timestamp:dataTimestamp,universe_mode:mode,eligible_universe:universe,engine_version:row.engine,strategy_version:row.strategy,quant_version:row.quant,ml_version:row.ml,feature_version:row.feature,source_dataset_hash:sourceHash,scan_cadence_ms:cadence,candidate_count:candidateCount});
  if(!row.engine||!row.strategy||!row.quant||!row.ml||!row.feature||row.snapshotHash!==expected)throw Object.assign(new Error('Historical snapshot provenance hash mismatch'),{status:409,code:'HISTORICAL_RANK_SNAPSHOT_HASH_MISMATCH'});
  return row;
}
function historicalRankCandidate(input,scan){
  const id=safeText(input?.candidate_id,180),asset=safeAsset(input?.asset),direction=safeText(input?.direction,12),strategy=safeText(input?.strategy,120),number=key=>Number(input?.[key]),valid=input?.valid_current_geometry===true||Number(input?.valid_current_geometry)===1,candidateCount=Number(input?.candidate_count),rank=input?.candidate_rank==null?null:Number(input.candidate_rank),features=input?.feature_json&&typeof input.feature_json==='object'?input.feature_json:{},featureHash=safeText(input?.feature_hash,120),sequence=input?.sequence_json&&typeof input.sequence_json==='object'?input.sequence_json:null,sequenceHash=safeText(input?.sequence_hash,120),sequenceVersion=safeText(input?.sequence_version,80),candidateHash=safeText(input?.candidate_hash,120),ml=safeText(input?.ml_applicability,100);
  const invalidFields=[];if(!id)invalidFields.push('id');if(!['BTC','ETH','SOL','XRP','DOGE','LTC'].includes(asset))invalidFields.push('asset');if(!['long','short'].includes(direction))invalidFields.push('direction');if(!strategy)invalidFields.push('strategy');if(candidateCount!==scan.candidateCount)invalidFields.push('candidate_count');if(!featureHash)invalidFields.push('feature_hash');if(!candidateHash)invalidFields.push('candidate_hash');if((sequence&&!sequenceHash)||(!sequence&&!sequenceHash&&sequenceVersion)||(!sequence&&!sequenceHash&&scan.engine==='HISTORICAL-RANK-PILOT-V2')||sequence&&sequenceVersion!=='candle-sequence-v1')invalidFields.push('sequence');if(ml!=='ML_NOT_AVAILABLE_FOR_HISTORICAL_TIMESTAMP')invalidFields.push('ml_applicability');if(rank!==null&&(!Number.isInteger(rank)||rank<1||rank>candidateCount))invalidFields.push('candidate_rank');
  if(invalidFields.length)throw Object.assign(new Error(`Invalid historical rank candidate fields: ${invalidFields.join(',')}`),{status:400,code:'HISTORICAL_RANK_CANDIDATE_INVALID'});
  const entry=number('entry'),stop=number('stop'),tp1=number('tp1'),tp2=number('tp2'),rr=number('rr'),price=number('reference_price'),setup=number('setup_quality'),quant=number('quant_score'),combined=number('combined_score');
  const ordered=direction==='long'?stop<entry&&tp1>entry&&tp2>=tp1:stop>entry&&tp1<entry&&tp2<=tp1;
  if(valid!==Boolean(ordered&&Number.isFinite(rr)&&rr>0)||(valid&&![setup,quant,combined].every(Number.isFinite)))throw Object.assign(new Error('Historical candidate geometry is inconsistent'),{status:400,code:'HISTORICAL_RANK_GEOMETRY_INVALID'});
  if(valid!==Boolean(rank!==null))throw Object.assign(new Error('Only valid historical candidates may be ranked'),{status:400,code:'HISTORICAL_RANK_RANK_INVALID'});
  const row={id,scanId:scan.scanId,asset,instrument:safeText(input?.invo_instrument,40)||null,direction,strategy,price:Number.isFinite(price)?price:null,entry:Number.isFinite(entry)?entry:null,stop:Number.isFinite(stop)?stop:null,tp1:Number.isFinite(tp1)?tp1:null,tp2:Number.isFinite(tp2)?tp2:null,rr:Number.isFinite(rr)?rr:null,setup:Number.isFinite(setup)?setup:null,entryQuality:safeText(input?.entry_quality,40)||null,quant:Number.isFinite(quant)?quant:null,mlRaw:Number.isFinite(number('ml_raw_score'))?number('ml_raw_score'):null,combined:Number.isFinite(combined)?combined:null,regime:safeText(input?.regime,100)||'UNCLASSIFIED',features,featureHash,sequence,sequenceHash,sequenceVersion,rank,candidateCount,valid,invalidation:safeText(input?.invalidation_reason,500)||null,candidateHash};
  const expected=historicalRankHash({...input,targets:undefined,candidate_hash:undefined});
  if(expected!==candidateHash)throw Object.assign(new Error('Historical candidate provenance hash mismatch'),{status:409,code:'HISTORICAL_RANK_CANDIDATE_HASH_MISMATCH'});
  return row;
}
async function historicalRankSnapshotCommit(payload,env,now){
  const snapshot=historicalRankSnapshot(payload),raw=Array.isArray(payload?.candidates)?payload.candidates:[];
  if(raw.length!==snapshot.candidateCount||new Set(raw.map(item=>safeText(item?.candidate_id,180))).size!==raw.length)throw Object.assign(new Error('Historical candidate universe is incomplete or duplicated'),{status:400,code:'HISTORICAL_RANK_UNIVERSE_INVALID'});
  const candidates=raw.map(item=>historicalRankCandidate(item,snapshot)),ranks=candidates.filter(row=>row.rank!==null).map(row=>row.rank);
  if(new Set(ranks).size!==ranks.length||ranks.some((rank,index)=>rank!==index+1))throw Object.assign(new Error('Historical candidate ranks must be a contiguous frozen order'),{status:400,code:'HISTORICAL_RANK_ORDER_INVALID'});
  const existing=await env.MARKET_EDGE_DB.prepare(`SELECT snapshot_hash FROM historical_scan_snapshots WHERE scan_id=?`).bind(snapshot.scanId).first();
  if(existing&&existing.snapshot_hash!==snapshot.snapshotHash)throw Object.assign(new Error('Historical scan ID already exists with different immutable content'),{status:409,code:'HISTORICAL_RANK_IMMUTABLE_CONFLICT'});
  const statements=[];
  if(!existing)statements.push(env.MARKET_EDGE_DB.prepare(`INSERT INTO historical_scan_snapshots (scan_id,scan_timestamp,data_timestamp,universe_mode,eligible_universe_json,engine_version,strategy_version,quant_version,ml_version,feature_version,source_dataset_hash,scan_cadence_ms,candidate_count,snapshot_hash,created_at,immutable) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)`).bind(snapshot.scanId,snapshot.timestamp,snapshot.dataTimestamp,snapshot.mode,JSON.stringify(snapshot.universe),snapshot.engine,snapshot.strategy,snapshot.quant,snapshot.ml,snapshot.feature,snapshot.sourceHash,snapshot.cadence,snapshot.candidateCount,snapshot.snapshotHash,now));
  for(const row of candidates){statements.push(env.MARKET_EDGE_DB.prepare(`INSERT OR IGNORE INTO historical_scan_candidates (candidate_id,scan_id,asset,invo_instrument,direction,strategy,reference_price,entry,stop,tp1,tp2,rr,setup_quality,entry_quality,quant_score,ml_applicability,ml_raw_score,combined_score,regime,feature_json,feature_hash,candidate_rank,candidate_count,valid_current_geometry,invalidation_reason,targets_json,candidate_hash,created_at,immutable) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1)`).bind(row.id,row.scanId,row.asset,row.instrument,row.direction,row.strategy,row.price,row.entry,row.stop,row.tp1,row.tp2,row.rr,row.setup,row.entryQuality,row.quant,'ML_NOT_AVAILABLE_FOR_HISTORICAL_TIMESTAMP',row.mlRaw,row.combined,row.regime,JSON.stringify(row.features),row.featureHash,row.rank,row.candidateCount,row.valid?1:0,row.invalidation,JSON.stringify({status:'PENDING_OUTCOME'}),row.candidateHash,now));if(row.valid&&row.sequence)statements.push(env.MARKET_EDGE_DB.prepare(`INSERT OR IGNORE INTO historical_candidate_sequences (candidate_id,scan_id,signal_timestamp,asset,sequence_version,source_dataset_hash,sequence_hash,sequence_json,created_at,immutable) VALUES (?,?,?,?,?,?,?,?,?,1)`).bind(row.id,row.scanId,snapshot.timestamp,row.asset,row.sequenceVersion,snapshot.sourceHash,row.sequenceHash,JSON.stringify(row.sequence),now));}
  // Replaying an already-frozen empty universe is a legitimate idempotent
  // no-op. D1 rejects batch([]), so do not turn that into a false data error.
  if(statements.length)await env.MARKET_EDGE_DB.batch(statements);
  return {accepted:true,operation:'historical_rank_snapshot_commit',scan_id:snapshot.scanId,candidate_count:candidates.length,ranked_count:ranks.length,immutable:true,ml:'ML_NOT_AVAILABLE_FOR_HISTORICAL_TIMESTAMP'};
}
function historicalRankTarget(input){
  const id=safeText(input?.candidate_id,180),target=input?.targets&&typeof input.targets==='object'?input.targets:{},finalR=Number(target.FINAL_R),mfe=Number(target.MFE),mae=Number(target.MAE),duration=Number(target.duration_bars),hash=safeText(input?.outcome_hash,120);
  const resolved=target.status==='RESOLVED'&&[finalR,mfe,mae,duration].every(Number.isFinite)&&duration>=1&&typeof target.TP1_BEFORE_SL==='boolean'&&typeof target.STOP_HIT==='boolean'&&typeof target.TP2_HIT==='boolean';
  const dataGap=target.status==='UNRESOLVED_DATA_GAP'&&Boolean(safeText(target.reason,120))&&(target.next_valid_candle_gap_ms==null||Number.isFinite(Number(target.next_valid_candle_gap_ms)));
  if(!id||(!resolved&&!dataGap)||!hash||hash!==historicalRankHash({candidate_id:id,targets:target}))throw Object.assign(new Error('Invalid historical rank outcome'),{status:400,code:'HISTORICAL_RANK_OUTCOME_INVALID'});
  return {id,target,hash,status:target.status};
}
async function historicalRankOutcomeCommit(payload,env,now){
  ...
  if(!scanId||!targets.length||targets.length>96||new Set(targets.map(row=>row.id)).size!==targets.length)throw Object.assign(new Error('Invalid historical rank outcome batch'),{status:400,code:'HISTORICAL_RANK_OUTCOME_BATCH_INVALID'});
...
async function mlIngest(request,env,payload,now=Date.now()){
  if(payload?.operation==='ml_generation_commit')return mlGenerationIngest(request,env,payload,now);if(payload?.operation==='ml_promotion_readiness_commit')return readinessCommit(request,env,payload,now);if(payload?.operation==='ml_shadow_resolve')return shadowResolve(request,env,payload,now);researchToken(request,env);if(payload?.operation!=='ml_research_commit')throw Object.assign(new Error('Unsupported ML research operation'),{status:400,code:'ML_INVALID_OPERATION'});
...
function validShadowSelection(value,{modelRequired=false}={}){
  if(!value||typeof value!=='object')return null;const asset=safeAsset(value.asset),direction=safeText(value.direction,12),strategy=safeText(value.strategy,120),regime=safeText(value.regime,100),entry=Number(value.entry),stop=Number(value.stop),target1=Number(value.target1),target2=Number(value.target2),quantScore=Number(value.quantScore),rawScore=Number(value.mlRawScore),combinedScore=Number(value.combinedScore),modelId=safeText(value.modelId,180),featureHash=safeText(value.featureHash,120);
  if(asset==='UNKNOWN'||!['long','short'].includes(direction)||!strategy||![entry,stop,target1,target2,quantScore,combinedScore].every(Number.isFinite)||!featureHash||(modelRequired&&!modelId))return null;
  if(direction==='long'&&!(stop<entry&&target1>entry&&target2>=target1))return null;if(direction==='short'&&!(stop>entry&&target1<entry&&target2<=target1))return null;
  return {asset,direction,strategy,regime:regime||'UNCLASSIFIED',entry,stop,target1,target2,quantScore,mlRawScore:Number.isFinite(rawScore)?rawScore:null,combinedScore,modelId:modelId||null,modelGeneration:Number.isFinite(Number(value.modelGeneration))?Number(value.modelGeneration):null,datasetHash:safeText(value.datasetHash,120)||null,featureHash};
}
async function lifecycleStatuses(db){const rows=(await db.prepare(`SELECT model_id,to_status,reason,report_id,created_at FROM ml_model_lifecycle_events ORDER BY created_at DESC`).all().catch(()=>({results:[]}))).results||[],latest=new Map();for(const row of rows)if(!latest.has(row.model_id))latest.set(row.model_id,row);return latest;}
async function shadowChallengers(db,env={}){const [models,lifecycle]=await Promise.all([db.prepare(`SELECT id,status,algorithm,dataset_hash,metadata_json,created_at FROM model_registry WHERE status='CHALLENGER' ORDER BY created_at DESC LIMIT 12`).all(),lifecycleStatuses(db)]),maximum=Math.max(1,Math.min(4,Number(env.MAX_ACTIVE_CHALLENGERS)||2));return(models?.results||[]).map(row=>({row,metadata:parseModelMetadata(row.metadata_json),event:lifecycle.get(row.id)})).filter(item=>!item.event||item.event.to_status==='CHALLENGER').filter(item=>item.metadata?.model&&Array.isArray(item.metadata.model.featureNames)).slice(0,maximum).map(item=>({id:item.row.id,status:item.event?.to_status||item.row.status,algorithm:item.row.algorithm,datasetHash:item.row.dataset_hash,createdAt:item.row.created_at,generation:Number(item.metadata.generation)||null,target:item.metadata.target||null,model:item.metadata.model,featureSchema:item.metadata.featureSchema||null}));}
async function shadowScanIngest(payload,env,now=Date.now()){
  const input=payload?.scan&&typeof payload.scan==='object'?payload.scan:{},scanId=safeText(input.scanId,180),challengerModelId=safeText(input.challengerModelId,180),timestamp=Number(input.timestamp),candidateUniverse=Array.isArray(input.candidateUniverse)?input.candidateUniverse:[],quant=validShadowSelection(input.quantOnly),incumbent=validShadowSelection(input.incumbent),challenger=validShadowSelection(input.challenger,{modelRequired:true});
  if(!scanId||!challengerModelId||!Number.isFinite(timestamp)||timestamp<=0||candidateUniverse.length<1||candidateUniverse.length>240||!quant||!incumbent||!challenger||challenger.modelId!==challengerModelId)throw Object.assign(new Error('Invalid frozen challenger shadow scan'),{status:400,code:'SHADOW_SCAN_INVALID'});
  const eligible=(await shadowChallengers(env.MARKET_EDGE_DB,env)).find(model=>model.id===challengerModelId);if(!eligible)throw Object.assign(new Error('Challenger is not eligible for shadow evaluation'),{status:409,code:'SHADOW_CHALLENGER_INACTIVE'});
  await env.MARKET_EDGE_DB.prepare(`INSERT OR IGNORE INTO ml_challenger_shadow_scans (scan_id,challenger_model_id,timestamp,candidate_universe_json,quant_selection_json,incumbent_selection_json,challenger_selection_json,outcome_json,created_at,resolved_at,immutable) VALUES (?,?,?,?,?,?,?,NULL,?,NULL,1)`).bind(scanId,challengerModelId,timestamp,JSON.stringify(candidateUniverse),JSON.stringify(quant),JSON.stringify(incumbent),JSON.stringify(challenger),now).run();
  return {accepted:true,scanId,challengerModelId,status:'FORWARD / PENDING',immutable:true};
}
function validShadowOutcome(value){const finalR=Number(value?.finalR),netUtility=Number(value?.netUtility??value?.finalR),tp1=value?.tp1BeforeSl;if(!Number.isFinite(finalR)||!Number.isFinite(netUtility)||(tp1!==true&&tp1!==false))return null;return{finalR,netUtility,tp1BeforeSl:tp1,mfe:Number.isFinite(Number(value?.mfe))?Number(value.mfe):null,mae:Number.isFinite(Number(value?.mae))?Number(value.mae):null,breakoutFailure:value?.breakoutFailure===true?true:value?.breakoutFailure===false?false:null,estimatedCosts:Number.isFinite(Number(value?.estimatedCosts))?Number(value.estimatedCosts):null,rawScore:Number.isFinite(Number(value?.rawScore))?Number(value.rawScore):null};}
async function shadowResolve(request,env,payload,now=Date.now()){
  researchToken(request,env);const scanId=safeText(payload?.scan_id,180),modelId=safeText(payload?.challenger_model_id,180),source=payload?.outcome&&typeof payload.outcome==='object'?payload.outcome:{},outcome={quantOnly:validShadowOutcome(source.quantOnly),incumbent:validShadowOutcome(source.incumbent),challenger:validShadowOutcome(source.challenger)};if(!scanId||!modelId||Object.values(outcome).some(value=>!value))throw Object.assign(new Error('Invalid comparable shadow outcome'),{status:400,code:'SHADOW_OUTCOME_INVALID'});const result=await env.MARKET_EDGE_DB.prepare(`UPDATE ml_challenger_shadow_scans SET outcome_json=?,resolved_at=? WHERE scan_id=? AND challenger_model_id=? AND outcome_json IS NULL`).bind(JSON.stringify(outcome),now,scanId,modelId).run();if(!Number(result?.meta?.changes||result?.changes||0))throw Object.assign(new Error('Frozen shadow scan was not pending'),{status:409,code:'SHADOW_OUTCOME_CONFLICT'});return{accepted:true,scanId,modelId,status:'RESOLVED'};
}
async function readinessSource(request,env){researchToken(request,env);const challengers=await shadowChallengers(env.MARKET_EDGE_DB,env),ids=challengers.map(model=>model.id);if(!ids.length)return{challengers:[],scans:[]};const scans=(await env.MARKET_EDGE_DB.prepare(`SELECT scan_id,challenger_model_id,timestamp,candidate_universe_json,quant_selection_json,incumbent_selection_json,challenger_selection_json,outcome_json,created_at,resolved_at FROM ml_challenger_shadow_scans ORDER BY timestamp ASC LIMIT 1000`).all()).results||[];const sourceModels=(await env.MARKET_EDGE_DB.prepare(`SELECT id,status,algorithm,dataset_hash,metadata_json,created_at FROM model_registry WHERE status='CHALLENGER'`).all()).results||[];return{challengers:challengers.map(model=>{const row=sourceModels.find(item=>item.id===model.id);return{...row,metadata:parseModelMetadata(row?.metadata_json)}}),scans:scans.filter(scan=>ids.includes(scan.challenger_model_id))};}
async function readinessCommit(request,env,payload,now=Date.now()){
  researchToken(request,env);const reports=Array.isArray(payload?.reports)?payload.reports:[],runId=safeText(payload?.run_id,180);if(!runId||!reports.length||reports.length>4)throw Object.assign(new Error('Invalid promotion readiness payload'),{status:400,code:'READINESS_INVALID'});const active=await shadowChallengers(env.MARKET_EDGE_DB,env),statements=[],saved=[];for(const report of reports){const modelId=safeText(report?.modelId,180),decision=safeText(report?.decision,32),model=active.find(item=>item.id===modelId);if(!model||!['INSUFFICIENT_EVIDENCE','KEEP_CHALLENGER','PROMOTION_READY','REJECTED'].includes(decision)||report?.status!==decision||!report?.hardGates||!report?.evidence)throw Object.assign(new Error('Promotion readiness report is invalid'),{status:400,code:'READINESS_REPORT_INVALID'});const reportId=`readiness-${modelId}-${stableHash({decision,n:report.forwardResolvedN,days:report.forwardDays,reasons:report.decisionReasons,createdAt:report.createdAt})}`,reason=safeText((report.decisionReasons||[]).join('; '),500)||'Deterministic readiness evaluation';statements.push(env.MARKET_EDGE_DB.prepare(`INSERT OR IGNORE INTO ml_promotion_readiness_reports (report_id,model_id,report_json,created_at,immutable) VALUES (?,?,?,?,1)`).bind(reportId,modelId,JSON.stringify({...report,reportId,updatedAt:now}),now));if(['PROMOTION_READY','REJECTED'].includes(decision)){const eventId=`lifecycle-${modelId}-${decision}-${stableHash(reportId)}`;statements.push(env.MARKET_EDGE_DB.prepare(`INSERT OR IGNORE INTO ml_model_lifecycle_events (event_id,model_id,from_status,to_status,reason,report_id,created_at,immutable) VALUES (?,?,?,?,?,?,?,1)`).bind(eventId,modelId,'CHALLENGER',decision,reason,reportId,now));}saved.push({modelId,decision,reportId});}await env.MARKET_EDGE_DB.batch(statements);return{accepted:true,saved,autoChampionPromotion:false};
}
async function mlEvolution(db,env={}){const [active,challengers,reports,lifecycle]=await Promise.all([activeMlModel(db),shadowChallengers(db,env),db.prepare(`SELECT report_id,model_id,report_json,created_at FROM ml_promotion_readiness_reports ORDER BY created_at DESC LIMIT 30`).all().catch(()=>({results:[]})),lifecycleStatuses(db)]),latestReports=new Map();for(const row of reports?.results||[])if(!latestReports.has(row.model_id))latestReports.set(row.model_id,{reportId:row.report_id,...parseModelMetadata(row.report_json),createdAt:row.created_at});const weight=active.status==='CHAMPION'&&active.calibration?.status==='APPLIED'?'25%':active.status==='SHADOW'?'10%':active.available?'5%':'0%',generation=Number((String(active.id||'').match(/GEN(\d+)/i)||[])[1])||null;return{active:{id:active.id||null,status:active.status||'UNAVAILABLE',generation,weight},challengers:challengers.map(model=>({id:model.id,status:lifecycle.get(model.id)?.to_status||model.status,generation:model.generation,datasetHash:model.datasetHash,report:latestReports.get(model.id)||null})),autoChampionPromotion:false};}
...
function entitlementAfterFinalization(row) {
  const result=entitlement(row);
  if (!result.unlimited) result.usedToday+=1;
  return result;
}
function entitlementAfterRelease(row) {
  const result=entitlement(row);
  if (!result.unlimited) result.remainingToday=Math.min(result.dailyLimit,result.remainingToday+1);
  return result;
}
async function entitledCustomerScan(request,env,payload,fetchImpl,now) {
  const principal=await resolvePrincipal(request,env,fetchImpl);
  const requestId=safeText(payload?.requestId||payload?.scan_id||crypto.randomUUID(),160);
  if (requestId.length<8) throw accountError('A valid scan request ID is required.', 'REQUEST_ID_INVALID', 400);
  const reservation=await reserveScan(principal,requestId,env,fetchImpl,now);
  if (!reservation.allowed) throw Object.assign(accountError('Daily scan limit reached.', 'SCAN_LIMIT_REACHED', 429),{entitlement:entitlement(reservation),setCookie:principal.setCookie});
  try {
    const scan=await customerScan(env,payload,fetchImpl,now);
    if (!successfulScan(scan)) {
      await releaseScan(reservation.reservation_id,env,fetchImpl);
...
export async function handleRequest(request,env={},ctx={},deps={}) {
  ...
  if(request.method==='GET'&&url.pathname==='/v1/account') {
    try { const principal=await resolvePrincipal(request,env,fetchImpl); const data={principalType:principal.type,email:principal.email,displayName:principal.profile?.displayName||null,...await accountState(principal,env,fetchImpl,deps.now||Date.now())}; return json(data,200,{...cors,...(principal.setCookie?{'set-cookie':principal.setCookie}:{})}); }
    catch(error) { return json({error:{code:error.code||'ACCOUNT_ERROR',message:safeText(error.message,240)}},error.status||400,cors); }
  }
  if(request.method==='GET'&&url.pathname==='/v1/trades') {
    try { const principal=await resolvePrincipal(request,env,fetchImpl); const data=await listTrades(principal,env,fetchImpl); return json({trades:data},200,{...cors,...(principal.setCookie?{'set-cookie':principal.setCookie}:{})}); }
    catch(error) { return json({error:{code:error.code||'ACCOUNT_ERROR',message:safeText(error.message,240)}},error.status||400,cors); }
  }
  ...
  if(request.method==='GET'&&url.pathname==='/v1/research/ml/challengers') return json({challengers:await shadowChallengers(env.MARKET_EDGE_DB,env),maxActive:Math.max(1,Math.min(4,Number(env.MAX_ACTIVE_CHALLENGERS)||2))},200,cors);
  ...
  const closingTrade=url.pathname.match(/^\/v1\/trades\/([0-9a-f-]{36})\/close$/i);
  if(request.method!=='POST'||!['/v1/scan','/api/scan','/v1/trades/take','/v1/analyze','/v1/chat','/v1/tradingview-alert','/v1/research/ingest','/v1/research/ml/ingest','/v1/research/ml/shadow-scan','/v1/research/forward-selections','/v1/community/paper-events'].includes(url.pathname)&&!closingTrade) return json({error:{code:'NOT_FOUND',message:'Endpoint not found'}},404,cors);
  ...
  try {
    let data,headers={...cors,'x-ratelimit-remaining':String(rate.remaining)};
    if(['/v1/scan','/api/scan'].includes(url.pathname)) { data=await entitledCustomerScan(request,env,payload,fetchImpl,deps.now||Date.now()); if(data._setCookie){headers['set-cookie']=data._setCookie;delete data._setCookie;} }
    else if(url.pathname==='/v1/trades/take') { const principal=await resolvePrincipal(request,env,fetchImpl); data=await acceptTrade(principal,payload?.recommendationId,env,fetchImpl); if(principal.setCookie)headers['set-cookie']=principal.setCookie; }
    else if(closingTrade) { const principal=await resolvePrincipal(request,env,fetchImpl); data={trade:await closeTrade(principal,closingTrade[1],payload?.exitPrice,env,fetchImpl,deps.now||Date.now())}; if(principal.setCookie)headers['set-cookie']=principal.setCookie; }
    else data=url.pathname==='/v1/analyze'?await analyze(request,env,payload,fetchImpl):url.pathname==='/v1/chat'?await chat(request,env,payload,fetchImpl):url.pathname==='/v1/tradingview-alert'?await acceptTradingViewAlert(request,env,payload,deps.now||Date.now()):url.pathname==='/v1/research/ml/ingest'?await mlIngest(request,env,payload,deps.now||Date.now()):url.pathname==='/v1/research/ml/shadow-scan'?await shadowScanIngest(payload,env,deps.now||Date.now()):url.pathname==='/v1/research/forward-selections'?await forwardSelectionIngest(payload,env,deps.now||Date.now()):await researchIngest(request,env,payload,deps.now||Date.now());
    ...
    return json({error:{code,message:safeText(error.message||'Request failed',240),entitlement:error.entitlement||undefined}},status,{...cors,...(error.setCookie?{'set-cookie':error.setCookie}:{})});
  }
}
