...
function historicalRankTarget(input){
  ...
  if(!id||target.status!=='RESOLVED'||![finalR,mfe,mae,duration].every(Number.isFinite)||duration<1||typeof target.TP1_BEFORE_SL!=='boolean'||typeof target.STOP_HIT!=='boolean'||typeof target.TP2_HIT!=='boolean'||!hash||hash!==historicalRankHash({candidate_id:id,targets:target}))throw Object.assign(new Error('Invalid historical rank outcome'),{status:400,code:'HISTORICAL_RANK_OUTCOME_INVALID'});
  return {id,target,hash};
}
async function historicalRankOutcomeCommit(payload,env,now){
  ...
  let resolved=0;for(const row of targets){const result=await env.MARKET_EDGE_DB.prepare(`UPDATE historical_scan_candidates SET targets_json=?,resolved_at=? WHERE candidate_id=? AND scan_id=? AND targets_json LIKE '%PENDING_OUTCOME%'`).bind(JSON.stringify(row.target),now,row.id,scanId).run();resolved+=Number(result?.meta?.changes)||0;}
  return {accepted:true,operation:'historical_rank_outcome_commit',scan_id:scanId,resolved,immutable:true};
}
