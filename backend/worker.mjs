...
async function mlIngest(request,env,payload,now=Date.now()){
  researchToken(request,env);if(payload?.operation!=='ml_research_commit')throw Object.assign(new Error('Unsupported ML research operation'),{status:400,code:'ML_INVALID_OPERATION'});
...
async function researchStatus(db){const [progress,run,models,shadow]=await Promise.all([replayProgress(db),db.prepare(`SELECT id,runner,stage,asset,status,input_cursor,output_cursor,candles_processed,decision_points_written,outcomes_resolved,detail_json,started_at,completed_at FROM research_runs ORDER BY created_at DESC LIMIT 1`).first().catch(()=>null),db.prepare(`SELECT id,status,algorithm,dataset_hash,metadata_json,created_at FROM model_registry ORDER BY created_at DESC LIMIT 40`).all().catch(()=>({results:[]})),db.prepare(`SELECT COUNT(*) AS total,SUM(CASE WHEN outcome_json IS NOT NULL THEN 1 ELSE 0 END) AS resolved FROM ml_shadow_predictions`).first().catch(()=>null)]),modelRows=models?.results||[],counts=Object.fromEntries(['REJECTED','RESEARCH','SHADOW','CHALLENGER','CHAMPION','RETIRED'].map(status=>[status,modelRows.filter(model=>model.status===status).length])),currentShadow=modelRows.find(model=>model.status==='SHADOW')||null,currentChampion=modelRows.find(model=>model.status==='CHAMPION')||null,mlStatus=currentChampion?'CHAMPION':currentShadow?'SHADOW':counts.RESEARCH?'RESEARCH':'DISABLED',recentModels=modelRows.slice(0,6).map(model=>{let metadata={};try{metadata=JSON.parse(model.metadata_json||'{}');}catch{}return{id:model.id,status:model.status,algorithm:model.algorithm,datasetHash:model.dataset_hash,createdAt:model.created_at,target:metadata.target||null,oosN:Number(metadata.oosN)||0,metrics:metadata.metrics||null,baselineComparison:metadata.baselineComparison||null,calibration:metadata.calibration||null};});return {...progress,lastResearchRun:run||null,runner:'github-actions-node',ml:{status:mlStatus,sampleN:progress.totalResolvedTargets,models:counts,recentModels,currentShadow:currentShadow?{id:currentShadow.id,algorithm:currentShadow.algorithm,datasetHash:currentShadow.dataset_hash}:null,currentChampion:currentChampion?{id:currentChampion.id,algorithm:currentChampion.algorithm,datasetHash:currentChampion.dataset_hash}:null,shadowPredictions:Number(shadow?.total)||0,resolvedShadowPredictions:Number(shadow?.resolved)||0}};}
...
export async function handleRequest(request,env={},ctx={},deps={}) {
  ...
  if(request.method!=='POST'||!['/v1/analyze','/v1/chat','/v1/tradingview-alert','/v1/research/ingest','/v1/research/ml/ingest'].includes(url.pathname)) return json({error:{code:'NOT_FOUND',message:'Endpoint not found'}},404,cors);
