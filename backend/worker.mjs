...
function historicalRankCandidate(input,scan){
  ...
  if(valid!==Boolean(ordered&&Number.isFinite(rr)&&rr>0)||!Number.isFinite(setup)||!Number.isFinite(quant)||!Number.isFinite(combined))throw Object.assign(new Error('Historical candidate geometry is inconsistent'),{status:400,code:'HISTORICAL_RANK_GEOMETRY_INVALID'});
  ...
  const row={id,scanId:scan.scanId,asset,instrument:safeText(input?.invo_instrument,40)||null,direction,strategy,price:Number.isFinite(price)?price:null,entry:Number.isFinite(entry)?entry:null,stop:Number.isFinite(stop)?stop:null,tp1:Number.isFinite(tp1)?tp1:null,tp2:Number.isFinite(tp2)?tp2:null,rr:Number.isFinite(rr)?rr:null,setup,entryQuality:safeText(input?.entry_quality,40)||null,quant,mlRaw:Number.isFinite(number('ml_raw_score'))?number('ml_raw_score'):null,combined,regime:safeText(input?.regime,100)||'UNCLASSIFIED',features,featureHash,rank,candidateCount,valid,invalidation:safeText(input?.invalidation_reason,500)||null,candidateHash};
