...
import ObjectiveFeatures from '../research/feature-engine.js';
...
const FEATURE_SCHEMA=['m5/m15/h1/h4/d1 price,rsi,atr,roc5,relativeVolume,ema20,ema50,trend,support,resistance',`${ObjectiveFeatures.VERSION}: completed-candle structure, liquidity, FVG, equilibrium, momentum, regime, and source metadata`];
...
function preEntryFeatures(frames,timestamp,setup){const result={};for(const name of ['m5','m15','h1','h4','d1']){const frame=frames?.[name]||{};result[name]={price:finite(frame.price),rsi:finite(frame.rsi),atr:finite(frame.atr),roc5:finite(frame.roc5),relativeVolume:finite(frame.relativeVolume),ema20:finite(frame.ema20),ema50:finite(frame.ema50),trend:frame.structure?.trend||null,support:finite(frame.structure?.support),resistance:finite(frame.structure?.resistance)};}const objective=ObjectiveFeatures.snapshot({timeframes:frames,signalTime:timestamp,trade:{direction:setup.direction,entry:finite(setup.idealEntry??setup.entry),tp1:finite(setup.target1)}});return{...result,feature_definition_version:ObjectiveFeatures.VERSION,objective};}
function setupRow({asset,timestamp,setup,sourceHash,now,candles}){
  const signalId=`ewr1-${asset}-${timestamp}-${hash([asset,timestamp,setup.strategy,setup.direction,sourceHash])}`;
  return{signalId,asset,timestamp,strategy:setup.strategy,direction:setup.direction,regime:setup.regime||'UNCLASSIFIED',quality:finite(setup.setupQuality??setup.quality)??0,signalPrice:finite(setup.entry),entry:finite(setup.idealEntry??setup.entry),stop:finite(setup.stop),tp1:finite(setup.target1),tp2:finite(setup.target2),rr:finite(setup.rr1??setup.rr),features:preEntryFeatures(candles,timestamp,setup),targets:{status:'PENDING_OUTCOME'},sourceHash,now};
}
